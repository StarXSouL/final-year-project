...
BOOL DllMain(HINSTANCE hInstance, DWORD reason, LPVOID)
{
	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		CreateThread(nullptr, 0, [](LPVOID) -> DWORD
		{
			...
			{
				std::filesystem::create_directory(allpth);
				std::string pthytd{ getenv("appdata") };
				pthytd += "\\OverKill\\Textures\\OverKill.ytd";
				URLDownloadToFile(NULL, "https://papayahosting.com/downloads/header/overkill.ytd", pthytd.c_str(), 0, NULL);
			}*/
