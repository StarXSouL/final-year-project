...
namespace Big
{
	...
	uintptr_t Memory::FindPointer(HANDLE hproc, uintptr_t ptr, std::vector<unsigned int> offsets)
	{
		uintptr_t addr = ptr;
...
#include <tlhelp32.h>
	...
	DWORD64 Memory::FindSignature(DWORD64 start, DWORD64 size, const char* sig, const char* mask)
	{
		BYTE* data = new BYTE[size];
		SIZE_T bytesRead;

		ReadProcessMemory(GetModuleHandle(NULL), (LPVOID)start, data, size, &bytesRead);

		for (DWORD64 i = 0; i < size; i++)
		{
			if (Memory::compare((const BYTE*)(data + i), (const BYTE*)sig, mask)) {
				return start + i;
			}
		}
		delete[] data;
	...
	bool Memory::compare(const uint8_t* pData, const uint8_t* bMask, const char* sMask)
	{
